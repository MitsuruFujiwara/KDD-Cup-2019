...
# get prediction with phase1 model
def getPrediction():

    # LightGBM
    pred_lgbm = np.zeros(df.shape[0])
    for n_fold in range(0,10):
        clf = lgb.Booster(model_file='../output/lgbm_{}.txt'.format(n_fold))

    # XGBoost
    for n_fold in range(0,5):
        clf = lgb.Booster(model_file='../output/xgb_{}.txt'.format(n_fold))

    return pred_lgbm, pred_xgb
